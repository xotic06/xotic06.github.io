...
La máquina Support fue muy divertida, el nivel de dificultad es bastante acertado pero si no se tienen los conocimientos necesarios tales como
revisar el trafico por wireshark puede ser un desafio, hay dos rutas para resolver esta máquina, la otra es haciendo reversing al .exe que nos
encontramos en el smbshare, este camino es un poco mas avanzado pero no es para comerse la cabeza tampoco. El tema que mas aprendi con este lab
fue lo de impersonar al usuario Administrator utilizando un computador falso. Sin nada mas que agregar, ojalá disfruten de la explicación!
